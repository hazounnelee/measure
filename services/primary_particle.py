...
# 유효 1차 입자 최소 면적 (미만이면 분석 대상에서 제외)
...
class PrimaryParticleService(Sam2AspectRatioService):
    ...
    def measure_primary_mask(
        ...
        """단일 mask를 minAreaRect로 측정하고 침상/판상으로 분류한다.
        면적이 area_threshold 미달이면 None을 반환한다.
        ...
        # 면적 미달 마스크는 1차입자로 분류하지 않고 제외한다.
        if int_maskArea < int(round(self.obj_config.float_particleAreaThreshold)):
            return None

        # AR 기준으로 acicular/plate 분류
        if float_aspectRatio < self.obj_primary_config.float_acicularThreshold:
    ...
    def _keep_primary_object(self, obj_m: PrimaryParticleMeasurement) -> bool:
        """파티클 타입별 분류 필터.

        acicular/plate 타겟이면 해당 카테고리(aspect_ratio 분류 결과)만 통과시킨다.
        'active' 등 형태 무관 타입은 모두 통과한다 (추후 타입별 기준 추가 지점).
        """
        str_target = self.obj_primary_config.str_particleType
        if str_target in ("acicular", "plate"):
            return obj_m.str_category == str_target
        return True
    ...
    def save_primary_outputs(
        ...
        # 개별 mask png (acicular_masks / plate_masks)
        for str_cat in ("acicular", "plate"):
    ...
    def process_primary(self) -> PrimaryParticleResult:
        ...
        for int_index, arr_mask in enumerate(arr_masks):
            ...
            if obj_m is None or not self._keep_primary_object(obj_m):
                continue

            # 최종 마스크: convex hull로 보정 후 재측정
            arr_hullMask = self._hull_mask(self.refine_mask_for_area(arr_mask))
            obj_final = self.measure_primary_mask(arr_hullMask, int_index, float_conf)
            if obj_final is None or not self._keep_primary_object(obj_final):
                continue

            list_objects.append(obj_final)
            list_validMasks.append(arr_hullMask.astype(np.uint8))
